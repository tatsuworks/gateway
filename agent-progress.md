...
- Known risk or unresolved issue: **Phase 3b sweep is not yet implemented and is now required for accuracy** — without it, a no-expiry skip lets disconnected-window ghosts accumulate (EXISTS-level drift) for guilds that are never re-backfilled. Removing offline-window departures fundamentally needs an RGM somewhere; the sweep is that avenue, paced and off the storm path.
- Next best step: design + implement the Phase 3b background sweep (manager-level goroutine: find guilds past `staleness × jitter`, route a paced RGM to the owning shard, reconcile). Then staging.
