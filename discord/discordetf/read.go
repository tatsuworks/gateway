...
func (d *genDecoder) NextTerm() (term Term, err error) {
...
func (d *genDecoder) read(n int) ([]byte, error) {
	if len(d.buf) < n {
		d.buf = make([]byte, n)
	}
...
func (d *genDecoder) readByte() (byte, error) {
	b, err := d.read(1)
	if err != nil {
		return 0, err
	}
...
func (d *genDecoder) readBigInt(l int, sign byte) (interface{}, error) {
	b, err := d.read(l)
	if err != nil {
		return nil, err
	}
...
func (d *genDecoder) readInt64(b []byte, sign byte) int64 {
	var result int64
	for i := 0; i < len(b); i++ {
...
func (d *genDecoder) ruint16() (uint16, error) {
	b, err := d.read(2)
	return be.Uint16(b), err
}

func (d *genDecoder) ruint32() (uint32, error) {
	b, err := d.read(4)
	return be.Uint32(b), err
}

// reads a byte and makes a byte slice with that size
func (d *genDecoder) buint8() ([]byte, error) {
	size, err := d.readByte()
...
func (d *genDecoder) uint8BorrowRead() ([]byte, error) {
	size, err := d.readByte()
	if err != nil {
		return nil, err
	}
...
func (d *genDecoder) buint16() ([]byte, error) {
	size, err := d.ruint16()
	return make([]byte, size), err
}
...
// the returned slice is reused so do not keep it around
func (d *genDecoder) uint16BorrowRead() ([]byte, error) {
	size, err := d.ruint16()
	if err != nil {
		return nil, err
	}
...
// reads 4 bytes as an uint and makes a byte slice with that size
func (d *genDecoder) buint32() ([]byte, error) {
	size, err := d.ruint32()
	return make([]byte, size), err
}
...
// the returned slice is reused so do not keep it around
func (d *genDecoder) uint32BorrowRead() ([]byte, error) {
	size, err := d.ruint32()
	if err != nil {
		return nil, err
	}
